...
public class ReportingServiceImpl implements ReportingService {
            ...
            members = p.getTeam().getMembers().stream()
                    .map(m -> {
                        ...
                        return new ManagerProjectMemberDto(
                                m.getId(),
                                ((m.getPrenom() != null ? m.getPrenom() : "") + " " + (m.getNom() != null ? m.getNom() : "")).trim(),
                                m.getEmail(),
                                profileLibelle,
                                0,
                                role
    ...
    public List<ProjectReportDto> getProjectsReport(Integer year, String status, Long teamId) {
        return projectRepository.findAll().stream()
                ...
                .filter(p -> {
                    ...
                    return p.getStartDate() != null && p.getStartDate().getYear() == year;
                })
                .filter(p -> {
                    if (status == null || status.isBlank()) return true;
                    return p.getStatus() != null && p.getStatus().name().equalsIgnoreCase(status);
                })
                .filter(p -> {
                    if (teamId == null) return true;
                    return p.getTeam() != null && p.getTeam().getId().equals(teamId);
                })
    ...
    @Override
    public List<TaskReportDto> getOverdueTasks(Long projectId) {
        LocalDate today = LocalDate.now();
    ...
    @Override
    public List<UserReportDto> getUsersWithoutProfile() {
    ...
    public UserStatusReportDto getUsersByStatus(Boolean active) {
        ...
        List<UserReportDto> filtered = allUsers.stream()
        ...
        UserReportDto topResetter = allUsers.stream()
    ...
    @Override
    public List<ProjectSelectDto> getProjectsForSelect() {
        return projectRepository.findAll().stream()
                .filter(p -> !p.isDeleted())
                .map(p -> new ProjectSelectDto(p.getId(), p.getName()))
                .collect(Collectors.toList());
    }

    @Override
    public List<TeamSelectDto> getTeamsForSelect() {
        return teamRepository.findAll().stream()
                .filter(t -> !t.isDeleted())
                .map(t -> new TeamSelectDto(t.getId(), t.getName()))
                .collect(Collectors.toList());
    }
    ...
    private TaskReportDto toTaskReportDto(Task t) {
