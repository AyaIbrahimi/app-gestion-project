// notification-bell-rc.component.ts
...
@Component({
  selector: 'app-notification-bell-rc',
  standalone: true,
      ...
      @if (isOpen) {
        <div class="dropdown">
          <div class="dropdown-header">
            <h3>Notifications - Responsable Contrat</h3>
            @if (unreadCount > 0) {
...
export class NotificationBellRcComponent implements OnInit, OnDestroy {
  isOpen = false;
  ...
  ngOnInit(): void {
    console.log('=== NotificationBellRcComponent INIT ===');
    this.loadNotifications();
    this.loadUnreadCount();
    
    this.subscriptions.push(
      this.notificationWS.notifications$.subscribe((notification: Notification) => {
        console.log('Nouvelle notification RC:', notification);
    ...
    this.subscriptions.push(
      this.notificationWS.unreadCount$.subscribe((count: number) => {
        console.log('Nouveau compteur RC:', count);
